...
inline void NaiveMul(const std::vector<double>& A,
...
inline std::vector<double> Sub(const std::vector<double>& X,
                               const std::vector<double>& Y) {
  assert(X.size() == Y.size());
  std::vector<double> R(X.size());
  for (std::size_t i = 0; i < X.size(); ++i) R[i] = X[i] - Y[i];
...
inline void Split(const std::vector<double>& M, std::size_t n,
...
inline std::vector<double> Join(const std::vector<double>& C11,
...
bool ZorinDStrassenAlgMatrixSEQ::ValidationImpl() {
  ...
  if (in.A.size() != in.n * in.n) return false;
  if (in.B.size() != in.n * in.n) return false;
