...
std::vector<int> KWayMergeChunks(const std::vector<int> &gathered, const std::vector<int> &counts,
  ...
  for (int i = 0; i < comm_size; ++i) {
    ...
    pq.push(Item{gathered[static_cast<size_t>(dsp)], i, 1});
  }
  std::vector<int> out;
  ...
  while (!pq.empty()) {
    ...
    if (x.next_j < cnt) {
      ...
      pq.push(Item{gathered[static_cast<size_t>(dsp + x.next_j)], x.chunk, x.next_j + 1});
    }
  }
  return out;
}
...
bool SmetaninDHoarSortALL::RunImpl() {
  ...
  if (local_n > 1) {
#pragma omp parallel default(none) shared(local)
#pragma omp single
