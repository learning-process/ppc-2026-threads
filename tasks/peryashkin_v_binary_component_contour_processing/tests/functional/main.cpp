...
BinaryImage BuildCase(int id) {
  switch (id) {
    ...
    case 2: {  // линия
      ...
      for (int x = 1; x <= 5; ++x) Set(im, x, 2, 1);
      return im;
    }
    case 3: {  // квадрат 3x3
      auto im = MakeEmpty(6, 6);
      for (int y = 2; y <= 4; ++y)
        for (int x = 2; x <= 4; ++x) Set(im, x, y, 1);
      return im;
    }
    case 4: {  // 2 компоненты
      auto im = MakeEmpty(8, 6);
      Set(im, 1, 1); Set(im, 2, 1); Set(im, 1, 2); Set(im, 2, 2);
      for (int y = 3; y <= 5; ++y) { Set(im, 6, y); Set(im, 7, y); }
      return im;
    }
    case 5: {  // “дырка” (кольцо 5x5)
      auto im = MakeEmpty(7, 7);
      for (int x = 1; x <= 5; ++x) { Set(im, x, 1); Set(im, x, 5); }
      for (int y = 1; y <= 5; ++y) { Set(im, 1, y); Set(im, 5, y); }
      return im;
    }
    case 6: {  // касается границы
      auto im = MakeEmpty(5, 5);
      for (int y = 0; y <= 2; ++y)
        for (int x = 0; x <= 1; ++x) Set(im, x, y, 1);
      return im;
    }
    default:
...
class PeryashkinVRunFuncTestsThreads
 ...
 protected:
  ...
  bool CheckTestOutputData(OutType& output_data) final {
    ...
    if (input_data_.data.empty()) return false;
...
const std::array<TestType, 7> kTestParam = {
    ...
    std::make_tuple(1, "point"),
    std::make_tuple(2, "line"),
    std::make_tuple(3, "square"),
    std::make_tuple(4, "two_components"),
    std::make_tuple(5, "hole"),
    std::make_tuple(6, "touch_border"),
};

const auto kTestTasksList =
    std::tuple_cat(
        ppc::util::AddFuncTask<PeryashkinVBinaryComponentContourProcessingSEQ, InType>(
