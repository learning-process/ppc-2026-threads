...
Умножение разреженных матриц — базовая операция в вычислительной линейной алгебре, широко применяемая в решении СЛАУ, конечно-элементных методах, графовых алгоритмах и машинном обучении. Использование разреженных форматов хранения позволяет:
...
```row_ptr``` заполняется нулями (позже будут вычислены смещения).

2. **Обход строк матрицы A:**
Для каждой строки ```i``` от ```0``` до ```A.rows-1```:
Извлекаются ненулевые элементы строки ```i``` из матрицы ```A```.

Создаётся ассоциативный контейнер ```std::map<int, double>``` для накопления результатов по столбцам ```C```.

3. **Внутреннее умножение:**
Для каждого ненулевого элемента ```(k, A_ik)``` из строки ```i```:
...
4. **Сохранение результатов строки:**
Из ```temp_result``` удаляются значения, близкие к нулю ```(|val| < eps)```.
Оставшиеся значения и индексы столбцов добавляются в ```values``` и ```col_index``` матрицы ```C```.
Обновляется ```row_ptr[i+1] = row_ptr[i] + (число ненулевых в строке)```.

5. **Завершение:**
Устанавливается финальный указатель ```row_ptr[A.rows] = values.size()```.

### Асимптотическая сложность

```nnz(A)``` - количество ненулевых элементов в A.
```nnz(B)``` - количество ненулевых элементов в B.
```nnz(C)``` - количество ненулевых элементов в C.
```n``` - размерность матриц.

**ВРЕМЯ:** ```O(nnz(A) * avg_nnz_per_row(B) + nnz(C))```.
В худшем случае: ```O(n^3)```. В разреженном случае значительно меньше.

**ПАМЯТЬ:** ```O(nnz(A) + nnz(B) + nnz(C) + A.rows)```.

### Критерий корректности

Результат умножения должен совпадать с результатом умножения плотных матриц, преобразованных из CRS-формата. Допустимая погрешность: ```e = 100 × DBL_EPSILON = 2.22e-14```.
...
- Метод ```ValidationImpl()``` - Проверка корректности входных данных (совместимость размерностей, положительность размеров).
...
1. **Использование ```std::map``` для накопления**:
```std::map<int, double>``` автоматически сортирует по ключу (столбцу) и обеспечивает уникальность столбцов. Это упрощает код, но добавляет логарифмический фактор ```O(log M)``` на каждую операцию вставки.

2. **Отфильтровка малых значений**:
Значения, по модулю меньшие ```e = 100 * DBL_EPSILON```, не включаются в результат. Это защищает от накопления численного шума.

3. **Два прохода по A (в закомментированной части)**:
В коде видна альтернативная реализация через ```MultiplyRowByMatrix```. В текущей версии используется прямой проход с ```std::map```, что более эффективно.

## 5. Проверка корректности

### Метод верификации

Результат умножения разреженных матриц сравнивается с эталоном — умножением плотных матриц. Для всех тестов выполняется проверка:
