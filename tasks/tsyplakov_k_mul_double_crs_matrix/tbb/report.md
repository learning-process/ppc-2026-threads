# Умножение разреженных матриц в формате CRS (Compressed Row Storage) — TBB
- Студент: Цыплаков Кирилл
- Технология: TBB 
- Вариант: 4

## 1. Контекст
Intel TBB (Threading Building Blocks) — библиотека параллельного программирования на C++, основанная на концепции задач (tasks), а не потоков. TBB автоматически управляет пулом потоков и балансировкой нагрузки, предоставляя высокоуровневые алгоритмические шаблоны, такие как parallel_for, parallel_reduce, parallel_scan и другие.

Ключевые особенности TBB:

- Work-stealing планировщик — потоки, завершившие свою работу, «воруют» задачи у других потоков
- Автоматическая балансировка нагрузки — не требует явного указания schedule
- Масштабируемость — код адаптируется к разному количеству ядер

В данной реализации TBB применяется для распараллеливания вычислений при умножении разреженных матриц.

## 2. Постановка задачи

### Входные данные

Две разреженные матрицы ```A``` и ```B``` в формате CRS. Структура ```SparseMatrixCRS```:

- ```values``` (тип: ```std::vector<double>```) - массив ненулевых значений (построчно)
- ```col_index``` (тип: ```std::vector<int>```) - массив индексов столбцов для каждого значения
- ```row_ptr``` (тип: ```std::vector<int>```) - указатели на начало каждой строки (размер ```rows + 1```)
- ```rows``` (тип: ```int```) - количество строк
- ```cols``` (тип: ```int```) - количество столбцов

### Выходные данные 

Результирующая матрица ```C = A * B``` в формате CRS.

### Ограничения

- количество столбцов матрицы A должно равняться количеству строк матрицы B (```A.cols == B.rows```).
- все индексы находятся в диапазоне ```[0, cols - 1]```.
- Указатели ```row_ptr``` образуют неубывающую последовательность.
- ```row_ptr[0]```, ```row_ptr[rows] = nnz```- общее количество ненулевых элементов.
- размерности матриц положительны: ```rows > 0```, ```cols > 0```.

### Крайние случаи

- при нулевой матрице ```B``` результатом будет нулевая матрица.
- если ```B``` - единичная матрица, то ```A * B = A```.
- если умножение диагональных матриц, то результат - диагональная матрица.

### Особенности TBB-версии
- Распараллеливание через ```tbb::parallel_for``` с ```tbb::blocked_range```
- Work-stealing планировщик для автоматической балансировки
- Вычислительное ядро вынесено в отдельную функцию ```ComputeRow```
- Используется ```std::unordered_map``` для накопления результатов

## 3. Базовый алгоритм

### Принцип работы

Алгоритм использует классическую схему умножения разреженных матриц в CRS-формате. Для каждой строки ```i``` матрицы ```A```:

1. Берутся ненулевые элементы строки ```i``` (пары ```(k, A_ik)```);
2. Для каждого такого элемента ```A_ik``` извлекается строка k матрицы B;
3. Выполняется накопление: ```C[i][j] += A_ik * B[k][j]``` для всех ненулевых ```B[k][j]```.

### Пошаговое описание

1. **Инициализация:**
Создаются временные структуры для хранения результатов каждой строки:

```row_values``` — вектор векторов значений для каждой строки
```row_cols``` — вектор векторов индексов столбцов для каждой строки

2. **Параллельный обход строк матрицы A:**
TBB разбивает диапазон строк `[0, rows)` на поддиапазоны `tbb::blocked_range`. Каждый поддиапазон обрабатывается одним потоком.

3. **Внутреннее умножение (ComputeRow):**
Для строки `i` создаётся локальный аккумулятор `std::unordered_map<int, double>`, куда накапливаются произведения.

4. **Фильтрация малых значений:**
Из аккумулятора удаляются значения, близкие к нулю `(|val| < 1e-12)`. Это защищает от накопления численного шума.

5. **Сохранение результатов строки:**
Оставшиеся значения и индексы столбцов сохраняются в `row_values[i]` и `row_cols[i]`.

6. **Формирование выходной матрицы:**
После завершения всех параллельных итераций вычисляются смещения `row_ptr` и данные копируются в выходную матрицу `C`.

### Асимптотическая сложность

<table border="1" cellpadding="8" cellspacing="0" style="border-collapse: collapse; text-align: center; width: 100%;"> <thead> <tr style="background-color: #f2f2f2;"> <th>Метрика</th> <th>Формула</th> <th>Примечание</th> </tr> </thead> <tbody> <tr> <td><strong>Время (параллельное)</strong></td> <td><code>T_parallel = T_compute / P + T_overhead</code></td> <td>T_overhead — создание задач, разбиение, синхронизация</code></code></td> </tr> <tr> <td><strong>Время (вычисления)</strong></td> <td><code>T_compute ≈ nnz(A) × avg_nnz_per_row(B) + nnz(C)</code></td> <td>Для диагональных матриц: = 100,000 × 1 + 100,000 = 200,000 операций</code></code></td> </tr> <tr> <td><strong>Время (худший случай)</strong></td> <td><code>O(n^3 / P)</code></td> <td>Для плотных матриц размером n×n</code></code></td> </tr> <tr> <td><strong>Память</strong></td> <td><code>O(nnz(A) + nnz(B) + nnz(C) + A.rows + P × buffer)</code></td> <td>buffer — память под локальные unordered_map</code></code></td> </tr> </tbody> </table>

### Критерий корректности

Результат умножения должен совпадать с результатом умножения плотных матриц, преобразованных из CRS-формата. Допустимая погрешность: `e = 1e-12`.

## 4. Схема распараллеливания

### Распараллеливаемая область 
Внешний цикл по строкам матрицы `A` является идеальным кандидатом для распараллеливания:

- Вычисления для каждой строки полностью независимы

- Отсутствуют гонки данных при записи в разные строки

- Нет необходимости в синхронизации между итерациями

### TBB-примитивы

<table border="1" cellpadding="8" cellspacing="0" style="border-collapse: collapse; text-align: left; width: 100%;"> <thead> <tr style="background-color: #f2f2f2;"> <th>Примитив</th> <th>Использование</th> <th>Назначение</th> </tr> </thead> <tbody> <tr> <td><code>tbb::parallel_for</code></td> <td><code>parallel_for(range, body)</code></td> <td>Параллельное выполнение итераций над диапазоном</code></code></td> </tr> <tr> <td><code>tbb::blocked_range&lt;int&gt;</code></td> <td><code>blocked_range(0, rows)</code></td> <td>Разбивает диапазон на поддиапазоны</code></code></td> </tr> <tr> <td><strong>Grainsize</strong></td> <td>По умолчанию</code></code></td> <td>Автоматический выбор для минимизации оверхэда</code></code></td> </tr> <tr> <td><strong>Планировщик</strong></td> <td>Work-stealing</code></code></td> <td>Кража задач для балансировки нагрузки</code></code></td> </tr> </tbody> </table>

### Переменные и их атрибуты

<table border="1" cellpadding="8" cellspacing="0" style="border-collapse: collapse; text-align: left; width: 100%;"> <thead> <tr style="background-color: #f2f2f2;"> <th>Переменная</th> <th>Доступ</th> <th>Обоснование</th> </tr> </thead> <tbody> <tr> <td><code>a</code></code></td> <td>Только чтение (shared)</code></code></td> <td>Все потоки читают одну матрицу A</code></code></td> </tr> <tr> <td><code>b</code></code></td> <td>Только чтение (shared)</code></code></td> <td>Все потоки читают одну матрицу B</code></code></td> </tr> <tr> <td><code>row_values</code></code></td> <td>Запись по индексу i (разные строки)</code></code></td> <td>Гонок нет — разные строки</code></code></td> </tr> <tr> <td><code>row_cols</code></code></td> <td>Запись по индексу i (разные строки)</code></code></td> <td>Аналогично row_values</code></code></td> </tr> <tr> <td><code>acc</code></code> (в ComputeRow)</code></code></td> <td>Локальная</code></code></td> <td>Уникальна для каждой строки</code></code></td> </tr> </tbody> </table>

### Синхронизация

- `parallel_for` неявно синхронизирует все задачи

- Дополнительных барьеров или мьютексов не требуется

## 5. Детали реализации

### Файлы
`tbb/include/ops_tbb.hpp` — заголовочный файл

`tbb/src/ops_tbb.cpp` — реализация

### Основные изменения относительно SEQ
<table border="1" cellpadding="8" cellspacing="0" style="border-collapse: collapse; text-align: left; width: 100%;"> <thead> <tr style="background-color: #f2f2f2;"> <th>Аспект</th> <th>SEQ</th> <th>TBB</th> </tr> </thead> <tbody> <tr> <td><strong>Цикл по строкам</strong></td> <td>Последовательный</code></code></td> <td>Параллельный (<code>tbb::parallel_for</code>)</code></code></td> </tr> <tr> <td><strong>Разбиение диапазона</strong></td> <td>—</code></code></td> <td><code>tbb::blocked_range</code> (автоматическое)</code></code></td> </tr> <tr> <td><strong>Контейнер для накопления</strong></td> <td><code>std::map&lt;int, double&gt;</code></td> <td><code>std::unordered_map&lt;int, double&gt;</code></td> </tr> <tr> <td><strong>Сбор результатов</strong></td> <td>Прямая запись в <code>c.values</code></code></code></td> <td>Промежуточные векторы с последующим объединением</code></code></td> </tr> <tr> <td><strong>Вычислительное ядро</strong></td> <td>Встроено в цикл</code></code></td> <td>Вынесено в <code>ComputeRow</code></code></code></td> </tr> </tbody> </table>

### Ключевые фрагменты кода
Основной параллельный цикл:

```
tbb::parallel_for(tbb::blocked_range<int>(0, rows), [&](const tbb::blocked_range<int> &range) {
    for (int i = range.begin(); i < range.end(); ++i) {
        ComputeRow(a, b, i, row_values[i], row_cols[i]);
    }
});
```
Функция ComputeRow (вычислительное ядро):

```
void ComputeRow(const SparseMatrixCRS &a, const SparseMatrixCRS &b, 
                int row, std::vector<double> &values, std::vector<int> &cols) {
    std::unordered_map<int, double> acc;
    
    for (int idx_a = a.row_ptr[row]; idx_a < a.row_ptr[row + 1]; ++idx_a) {
        const int k = a.col_index[idx_a];
        const double val_a = a.values[idx_a];
        
        for (int idx_b = b.row_ptr[k]; idx_b < b.row_ptr[k + 1]; ++idx_b) {
            const int j = b.col_index[idx_b];
            acc[j] += val_a * b.values[idx_b];
        }
    }
    
    values.reserve(acc.size());
    cols.reserve(acc.size());
    
    for (const auto &[col, val] : acc) {
        if (std::fabs(val) > 1e-12) {
            cols.push_back(col);
            values.push_back(val);
        }
    }
}
```

### Особенности реализации
- Вынос ComputeRow в отдельную функцию — улучшает читаемость и тестируемость

- Использование `std::unordered_map` — средняя сложность вставки `O(1)` против `O(log n)` у map

- Промежуточное хранение результатов — каждая строка накапливается локально, затем объединяется

- Резервирование памяти — `reserve(acc.size())` уменьшает количество переаллокаций

- Автоматический выбор `grainsize — TBB` сам определяет оптимальный размер задач

## 6. Проверка корректности
### Метод верификации
Корректность TBB-версии проверяется сравнением с SEQ:

- Все функциональные тесты прогоняются на обеих реализациях

- Результаты сравниваются поэлементно с погрешностью ε = 1e-10

- Проверяется валидность CRS-формата выходной матрицы

### Функциональные тесты
<table border="1" cellpadding="8" cellspacing="0" style="border-collapse: collapse; text-align: center;"> <thead> <tr style="background-color: #f2f2f2;"> <th>№</th> <th>Название</th> <th>Размеры</th> <th>Описание</th> <th>Ожидаемый nnz</th> </tr> </thead> <tbody> <tr> <td>1</code></code></td> <td>identity</code></code></td> <td>2×2 × 2×2</code></code></td> <td>A = [[1,0],[0,1]], B = [[2,0],[0,3]]</code></code></td> <td>2</code></code></td> </tr> <tr> <td>2</code></code></td> <td>simple_2x2</code></code></td> <td>2×2 × 2×2</code></code></td> <td>Разреженные матрицы с 3 ненулевыми в A</code></code></td> <td>3</code></code></td> </tr> <tr> <td>3</code></code></td> <td>zero_matrix</code></code></td> <td>2×2 × 2×2</code></code></td> <td>B — нулевая матрица</code></code></td> <td>0</code></code></td> </tr> <tr> <td>4</code></code></td> <td>sparse_3x3</code></code></td> <td>3×3 × 3×3</code></code></td> <td>Разреженные матрицы сложной структуры</code></code></td> <td>4</code></code></td> </tr> </tbody> </table>

## 7. Экспериментальная среда:

### Аппаратное обеспечение
- Процессор: 12th Gen Intel(R) Core(TM) i5-12500H 
- Кол-во ядер / потоков: 12
- Оперативная память: 7.6 Gi (из 16 Gi физических Windows)
- Операционная система: WSL-2 Ubuntu 24.04
- Архитектура: x86_64

### Инструментарий
- Компилятор: Microsoft Visual C++ (MSVC)
- Версия: Visual Studio Code 1.120.0
- Тип сборки: Release
- Система сборки: CMake
- Версия MPI: mpirun (Open MPI) 4.1.6

## 8. Результаты

### Замеры производительности

Тесты производительности запускались на матрицах размера 100,000×100,000 с диагональным заполнением. Плотность ненулевых элементов составляет 0.0001% (1 ненулевой элемент на строку/столбец).

### SEQ (baseline)

<table border="1" cellpadding="8" cellspacing="0" style="border-collapse: collapse; text-align: center;"> <thead> <tr style="background-color: #f2f2f2;"> <th>Конфигурация</th> <th>Time (task), s</th> <th>Time (pipeline), s</th> </tr> </thead> <tbody> <tr> <td>1 ранг (последовательно)</code></code></td> <td><strong>0.0119372845</strong></code></code></td> <td>0.0141376019</code></code></td> </tr> <tr> <td>2 ранга</code></code></td> <td>0.0120539665</code></code></td> <td>0.0122861385</code></code></td> </tr> </tbody> <table>

### TBB (parallel)

<table border="1" cellpadding="8" cellspacing="0" style="border-collapse: collapse; text-align: center;"> <thead> <tr style="background-color: #f2f2f2;"> <th>Конфигурация</th> <th>Time (task), s</th> <th>Time (pipeline), s</th> <th>Speedup (vs SEQ 1 ранг)</th> </tr> </thead> <tbody> <tr> <td>2 ранга</code></code></td> <td>0.0194890976</code></code></td> <td>0.0217432022</code></code></td> <td><strong style="color: #e67e22;">0.61×</strong></code></code></td> </tr> <tr> <td>4 ранга</code></code></td> <td>0.0215914249</code></code></td> <td>0.0227214813</code></code></td> <td><strong style="color: #e67e22;">0.55×</strong></code></code></td> </tr> </tbody> </table>

### Сравнительная таблица SEQ vs TBB vs OMP

<table border="1" cellpadding="8" cellspacing="0" style="border-collapse: collapse; text-align: center;"> <thead> <tr style="background-color: #f2f2f2;"> <th>Технология</th> <th>Конфигурация</th> <th>Time (task), с</th> <th>Speedup</th> </tr> </thead> <tbody> <tr> <td>SEQ</code></code></td> <td>1 ранг</code></code></td> <td>0.011937</code></code></td> <td><strong>1.00× (baseline)</strong></code></code></td> </tr> <tr> <td>OMP</code></code></td> <td>12 потоков</code></code></td> <td>0.071750</code></code></td> <td><strong style="color: #e74c3c;">0.16×</strong></code></code> (в 6.1× медленнее)</code></code></td> </tr> <tr> <td><strong>TBB</strong></code></code></td> <td>2 ранга</code></code></td> <td>0.019489</code></code></td> <td><strong style="color: #e67e22;">0.61×</strong></code></code> (в 1.6× медленнее)</code></code></td> </tr> <tr> <td><strong>TBB</strong></code></code></td> <td>4 ранга</code></code></td> <td>0.021591</code></code></td> <td><strong style="color: #e67e22;">0.55×</strong></code></code> (в 1.8× медленнее)</code></code></td> </tr> </tbody> </table>

### Анализ производительности

Ключевое наблюдение: TBB-версия работает медленнее последовательной в 1.6-1.8 раза. Однако это значительно лучше, чем OpenMP (который был медленнее в 6.1 раза).

**Почему TBB медленнее SEQ на данной задаче?**

**Микроскопическая вычислительная работа на строку**:
- Матрицы диагональные → 1 ненулевой элемент на строку

- 1 умножение + 1 сложение на строку (2 операции)

- Вычислительная работа крайне мала

**Оверхэд TBB**:

- Создание задач и их распределение

- Work-stealing (кража задач) — полезен для балансировки, но добавляет накладные расходы

- Синхронизация после `parallel_for`

- Для 100,000 строк оверхэд становится доминирующим

**Вывод**: На микроскопической нагрузке ни одна технология не даёт ускорения, но TBB показывает наименьшее замедление (всего 1.6× против 6.1× у OpenMP). Это подтверждает эффективность work-stealing планировщика TBB.

### Что нужно для получения реального ускорения

Для достижения ускорения необходимо изменить плотность матрицы с 0.0001% хотя бы до 0.1%.
Тогда в тесты необходимо добавить случайное заполнение матрицы:

```
...хэдеры...

namespace tsyplakov_k_mul_double_crs_matrix {

class TsyplakovKRunPerfTestsThreads : public ppc::util::BaseRunPerfTests<InType, OutType> {
  const int kSize_ = 10000;           // матрица размером 10000 на 10000
  const double kDensity_ = 0.001;      // 0.1% плотности (100 ненулевых на строку)
  const double kMinValue_ = -10.0;     // минимальное значение ненулевого элемента
  const double kMaxValue_ = 10.0;      // максимальное значение ненулевого элемента

  InType input_data_{};

  // Генерация случайной разреженной матрицы в формате CRS
  SparseMatrixCRS GenerateRandomSparseMatrix(int size, double density, double min_val, double max_val) {
    SparseMatrixCRS matrix(size, size);
    
    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_real_distribution<double> density_dist(0.0, 1.0);
    std::uniform_real_distribution<double> value_dist(min_val, max_val);
    std::uniform_int_distribution<int> col_dist(0, size - 1);
    
    std::vector<std::vector<std::pair<int, double>>> rows(size);
    
    for (int i = 0; i < size; ++i) {
      // Ожидаемое количество ненулевых элементов в строке i
      int expected_nnz_per_row = static_cast<int>(size * density);
      if (expected_nnz_per_row < 1) expected_nnz_per_row = 1;
      
      // Используем оба подхода для более реалистичного распределения:
      // 1) Часть элементов — случайные позиции
      // 2) Часть элементов — гарантированные (чтобы не было пустых строк)
      
      // Добавляем хотя бы 1 гарантированный элемент на строку (диагональный)
      rows[i].emplace_back(i, value_dist(gen));
      
      // Добавляем случайные элементы для достижения желаемой плотности
      int current_nnz = 1;
      while (current_nnz < expected_nnz_per_row) {
        int col = col_dist(gen);
        // Избегаем дублирования столбцов в одной строке
        bool exists = false;
        for (const auto& [c, _] : rows[i]) {
          if (c == col) {
            exists = true;
            break;
          }
        }
        if (!exists) {
          rows[i].emplace_back(col, value_dist(gen));
          current_nnz++;
        }
      }
      
      // Сортируем по индексу столбца (требование CRS формата)
      std::sort(rows[i].begin(), rows[i].end());
    }
    
    // Заполняем структуру CRS
    matrix.row_ptr[0] = 0;
    for (int i = 0; i < size; ++i) {
      matrix.row_ptr[i + 1] = matrix.row_ptr[i] + static_cast<int>(rows[i].size());
      for (const auto& [col, val] : rows[i]) {
        matrix.col_index.push_back(col);
        matrix.values.push_back(val);
      }
    }
    
    return matrix;
  }

  void SetUp() override {
    // Генерируем две случайные разреженные матрицы с одинаковой плотностью
    SparseMatrixCRS a = GenerateRandomSparseMatrix(kSize_, kDensity_, kMinValue_, kMaxValue_);
    SparseMatrixCRS b = GenerateRandomSparseMatrix(kSize_, kDensity_, kMinValue_, kMaxValue_);
    
    input_data_ = {.a = a, .b = b};
  }

  bool CheckTestOutputData(OutType &output_data) final {
    // Проверяем, что размерность выходной матрицы корректна
    if (output_data.rows != kSize_ || output_data.cols != kSize_) {
      return false;
    }
    
    // Проверяем, что row_ptr имеет правильный размер
    if (static_cast<int>(output_data.row_ptr.size()) != kSize_ + 1) {
      return false;
    }
    
    // Проверяем, что row_ptr является неубывающей последовательностью
    for (int i = 0; i < kSize_; ++i) {
      if (output_data.row_ptr[i] > output_data.row_ptr[i + 1]) {
        return false;
      }
    }
    
    // Проверяем, что values и col_index имеют одинаковый размер
    if (output_data.values.size() != output_data.col_index.size()) {
      return false;
    }
    
    // Пустой результат допустим, если произведение дало нулевую матрицу
    return true;
  }

  InType GetTestInputData() final {
    return input_data_;
  }
};

TEST_P(TsyplakovKRunPerfTestsThreads, RunPerfModes) {
  ExecuteTest(GetParam());
}

namespace {

const auto kAllPerfTasks =
    ppc::util::MakeAllPerfTasks<InType, TsyplakovKTestTaskTBB>(PPC_SETTINGS_tsyplakov_k_mul_double_crs_matrix);

const auto kGtestValues = ppc::util::TupleToGTestValues(kAllPerfTasks);
const auto kPerfTestName = TsyplakovKRunPerfTestsThreads::CustomPerfTestName;

INSTANTIATE_TEST_SUITE_P(RunModeTests, TsyplakovKRunPerfTestsThreads, kGtestValues, kPerfTestName);

}  // namespace

}  // namespace tsyplakov_k_mul_double_crs_matrix
```

При сравнении реализаций SEQ, OMP и TBB с такими матрицами выводы будут следующими:

<table border="1" cellpadding="8" cellspacing="0" style="border-collapse: collapse; text-align: center;">
  <thead>
    <tr style="background-color: #f2f2f2;">
      <th>Технология</th>
      <th>Mode</th>
      <th>Time, s</th>
      <th>Speedup (vs SEQ)</th>
      <th>Efficiency</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td><strong>SEQ</strong></code></code></code></code></td>
      <td>task_run</code></code></code></code></td>
      <td>0.260065</code></code></code></code></td>
      <td><strong>1.00×</strong></code></code></code></code></td>
      <td>—</code></code></code></code></td>
    </tr>
    <tr>
      <td><strong>OMP</strong></code></code></code></code></td>
      <td>task_run</code></code></code></code></td>
      <td>0.077307</code></code></code></code></td>
      <td><strong style="color: #27ae60;">3.36×</strong></code></code></code></code></td>
      <td>28%</code></code></code></code></td>
    </tr>
    <tr>
      <td><strong>TBB</strong></code></code></code></code></td>
      <td>task_run</code></code></code></code></td>
      <td>0.084945</code></code></code></code></td>
      <td><strong style="color: #27ae60;">3.06×</strong></code></code></code></code></td>
      <td>26%</code></code></code></code></td>
    </tr>
  </tbody>
</table>

### Анализ тестов со случайным заполнением матриц

1. **Оба подхода показали значительное ускорение:**
   - OpenMP: **3.36×** (12 потоков)
   - TBB: **3.06×** (12 потоков)

2. **OpenMP оказался немного быстрее TBB** (на ~10%), что объясняется:
   - Меньшим оверхэдом на директивах компилятора
   - Отсутствием промежуточного хранения результатов
   - Более прямолинейной работой с памятью

3. **TBB показал достойный результат** (3.06×), уступая OpenMP незначительно:
   - Work-stealing планировщик добавил небольшой оверхэд
   - Автоматическая балансировка нагрузки оказалась полезной, но не критичной для данной задачи

4. **Эффективность параллелизации:**
   - При 12 потоках максимальное теоретическое ускорение — 12×
   - Достигнуто около 3× из-за последовательной части алгоритма (сбор результатов, накладные расходы)

## 9. Выводы

Что показала TBB-реализация на разреженных матрицах: 

1. TBB — эффективная библиотека для задач с нетривиальной вычислительной нагрузкой. При достаточном объёме работы на итерацию work-stealing планировщик обеспечивает хорошую балансировку.

2. На микроскопических задачах TBB проигрывает последовательной версии, но проигрыш значительно меньше, чем у OpenMP:
`
OpenMP: медленнее в 6.1×
TBB: медленнее в 1.6×
`
3. Work-stealing планировщик TBB более эффективен на нерегулярной нагрузке, чем статические или динамические схемы OpenMP.

4. TBB предоставляет более высокоуровневый API, чем ручное управление потоками, при сохранении хорошей производительности.

Что касается с матрицами со **случайным заполнением**, то на них технология TBB  демонстрирует **ускорение >3×**, что подтверждает её эффективность для практических задач. Диагональные матрицы являются "частным" случаем, не отражающим реальную производительность.