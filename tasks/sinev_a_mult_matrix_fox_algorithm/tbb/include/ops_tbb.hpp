...
class SinevAMultMatrixFoxAlgorithmTBB : public BaseTask {
 ...
 private:
  ...
  // Методы (не static, чтобы можно было использовать TBB)
  void SimpleMultiply(size_t n, const std::vector<double> &a, const std::vector<double> &b, std::vector<double> &c);
  ...
  void MultiplyBlocks(const std::vector<double> &blocks_a, const std::vector<double> &blocks_b,
