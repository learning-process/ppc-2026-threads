...
KaurAMatrixMultiplySEQ::KaurAMatrixMultiplySEQ(const InType &in) {
  SetTypeOfTask(GetStaticTypeOfTask());
  GetInput() = in;
}

bool KaurAMatrixMultiplySEQ::ValidationImpl() {
...
bool KaurAMatrixMultiplySEQ::PreProcessingImpl() {
  return true;
}

bool KaurAMatrixMultiplySEQ::RunImpl() {
  const auto &matrix_left = std::get<0>(GetInput());
  const auto &matrix_right = std::get<1>(GetInput());
...
bool KaurAMatrixMultiplySEQ::PostProcessingImpl() {
  return true;
}
