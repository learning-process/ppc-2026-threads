...
KaurAMatrixMultiplySTL::KaurAMatrixMultiplySTL(const InType &in) {
  SetTypeOfTask(GetStaticTypeOfTask());
  GetInput() = in;
}

bool KaurAMatrixMultiplySTL::ValidationImpl() {
...
bool KaurAMatrixMultiplySTL::PreProcessingImpl() {
  return true;
}
...
bool KaurAMatrixMultiplySTL::RunImpl() {
  const auto &matrix_left = std::get<0>(GetInput());
  const auto &matrix_right = std::get<1>(GetInput());
...
bool KaurAMatrixMultiplySTL::PostProcessingImpl() {
  return true;
}
