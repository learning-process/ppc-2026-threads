...
KaurAMatrixMultiplyTBB::KaurAMatrixMultiplyTBB(const InType &in) {
  SetTypeOfTask(GetStaticTypeOfTask());
  GetInput() = in;
}

bool KaurAMatrixMultiplyTBB::ValidationImpl() {
...
bool KaurAMatrixMultiplyTBB::PreProcessingImpl() {
  return true;
}

bool KaurAMatrixMultiplyTBB::RunImpl() {
  const auto &matrix_left = std::get<0>(GetInput());
  const auto &matrix_right = std::get<1>(GetInput());
...
bool KaurAMatrixMultiplyTBB::PostProcessingImpl() {
  return true;
}
