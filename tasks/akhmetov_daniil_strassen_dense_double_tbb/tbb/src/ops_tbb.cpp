...
AkhmetovDStrassenDenseDoubleTBB::AkhmetovDStrassenDenseDoubleTBB(const InType &in) {
  SetTypeOfTask(GetStaticTypeOfTask());
  GetInput() = in;
}

bool AkhmetovDStrassenDenseDoubleTBB::ValidationImpl() {
...
bool AkhmetovDStrassenDenseDoubleTBB::PreProcessingImpl() {
  const auto &input = GetInput();
  const size_t n = format::GetN(input);
...
bool AkhmetovDStrassenDenseDoubleTBB::RunImpl() {
  const auto &input = GetInput();
...
bool AkhmetovDStrassenDenseDoubleTBB::PostProcessingImpl() {
  const auto &input = GetInput();
  const size_t n = format::GetN(input);
