 ...
 protected:
  void SetUp() override {
    ...
    if (func_name.find("constant") != std::string::npos) {
      ...
      func = [](const std::vector<double>& x) { 
        return x[0] * x[0] + (x.size() > 1 ? x[1] * x[1] : 0.0); 
      };
    } else if (func_name.find("product") != std::string::npos) {
      func = [](const std::vector<double>& x) { 
        double prod = 1.0;
        for (double val : x) prod *= val;
        return prod;
      };
    } else {
...
const std::array<TestType, 8> kTestCases = {
    // 1D
    std::make_tuple(std::vector<double>{0.0}, std::vector<double>{1.0}, 
                    std::vector<int>{10}, 1.0, "constant_1d"),
    std::make_tuple(std::vector<double>{0.0}, std::vector<double>{1.0}, 
                    std::vector<int>{10}, 0.5, "linear_1d"),
    std::make_tuple(std::vector<double>{0.0}, std::vector<double>{1.0}, 
                    std::vector<int>{10}, 1.0/3.0, "quadratic_1d"),
    
    // 2D
    std::make_tuple(std::vector<double>{0.0, 0.0}, std::vector<double>{1.0, 1.0}, 
                    std::vector<int>{10, 10}, 1.0, "constant_2d"),
    std::make_tuple(std::vector<double>{0.0, 0.0}, std::vector<double>{1.0, 1.0}, 
                    std::vector<int>{10, 10}, 0.25, "product_2d"),
    std::make_tuple(std::vector<double>{0.0, 0.0}, std::vector<double>{1.0, 1.0}, 
                    std::vector<int>{10, 10}, 2.0/3.0, "quadratic_2d"),
    
    // 3D
    std::make_tuple(std::vector<double>{0.0, 0.0, 0.0}, std::vector<double>{1.0, 1.0, 1.0}, 
                    std::vector<int>{8, 8, 8}, 1.0, "constant_3d"),
    std::make_tuple(std::vector<double>{0.0, 0.0, 0.0}, std::vector<double>{1.0, 1.0, 1.0}, 
                    std::vector<int>{8, 8, 8}, 0.125, "product_3d")
};
