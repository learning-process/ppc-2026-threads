  ...
  bool CheckTestOutputData(OutType &output_data) final {
    ...
    for (int ky = -1; ky <= 1; ++ky) {
      for (int kx = -1; kx <= 1; ++kx) {
        ...
        int py = cy + ky;
        if (px < 0) px = 0;
        if (px >= input_data_.width) px = input_data_.width - 1;
        if (py < 0) py = 0;
        if (py >= input_data_.height) py = input_data_.height - 1;
...
const auto kTestTasksList = ppc::util::AddFuncTask<TerekhovDGaussVertSEQ, InType>(
    kTestParam, PPC_SETTINGS_terekhov_d_seq_gauss_vert);
