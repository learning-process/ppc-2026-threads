#include "safaryan_a_sparse_matrix_mult_crs_all/all/include/ops_all.hpp"
...
#include "safaryan_a_sparse_matrix_mult_crs_all/common/include/common.hpp"
#include "util/include/util.hpp"

namespace safaryan_a_sparse_matrix_mult_crs_all {

SafaryanATaskALL::SafaryanATaskALL(const InType &in) {
  SetTypeOfTask(GetStaticTypeOfTask());
  GetInput() = in;
  GetOutput() = SparseMatrixCCS();
}

bool SafaryanATaskALL::IsMatrixValid(const SparseMatrixCCS &matrix) {
...
bool SafaryanATaskALL::ValidationImpl() {
...
bool SafaryanATaskALL::PreProcessingImpl() {
  const auto &[a, b] = GetInput();
...
SparseMatrixCCS SafaryanATaskALL::MultiplyMatrices(const SparseMatrixCCS &a, const SparseMatrixCCS &b) {
...
#pragma omp parallel for num_threads(chunks_count) default(none) shared(begin_cols, b, chunks_count)
...
bool SafaryanATaskALL::PostProcessingImpl() {
  return true;
}
