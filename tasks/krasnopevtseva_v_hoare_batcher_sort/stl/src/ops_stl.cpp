...
bool KrasnopevtsevaVHoareBatcherSortSTL::RunImpl() {
  ...
  // Параллельная сортировка блоков через std::async
  std::vector<std::future<void>> futures;
  for (int i = 0; i < numthreads; ++i) {
    futures.push_back(std::async(std::launch::async, [&res, &pointers, &sizes, i]() {
...
void KrasnopevtsevaVHoareBatcherSortSTL::BatcherMergeBlocksStep(int *left_pointer, int &left_size, 
...
void KrasnopevtsevaVHoareBatcherSortSTL::BatcherMerge(int thread_input_size, std::vector<int *> &pointers,
  ...
  for (int step = 1; pack > 1; step *= 2, pack /= 2) {
    ...
    if ((pack / 2) - 1 == 0) {
      BatcherMergeBlocksStep(pointers[0], sizes[sizes.size() - 1], 
                             pointers[pointers.size() - 1], sizes[sizes.size() - 1]);
    } else if ((pack / 2) % 2 != 0) {
