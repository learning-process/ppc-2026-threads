...
KaurADijkstraAlgSTL::KaurADijkstraAlgSTL(const InType &in) {
  SetTypeOfTask(GetStaticTypeOfTask());
  GetInput() = in;
  GetOutput() = 0;
}

bool KaurADijkstraAlgSTL::ValidationImpl() {
  return GetInput() > 0;
}

bool KaurADijkstraAlgSTL::PreProcessingImpl() {
  ...
  for (int thread_id = 0; thread_id < num_threads_; ++thread_id) {
    workers_[thread_id] = std::thread(&KaurADijkstraAlgSTL::WorkerLoop, this, thread_id);
  }

  return true;
}

void KaurADijkstraAlgSTL::DoFindMin(int thread_id) {
  const InType n = GetInput();
  const InType inf = std::numeric_limits<InType>::max();
...
void KaurADijkstraAlgSTL::DoRelax(int thread_id) {
  const InType n = GetInput();
  const InType inf = std::numeric_limits<InType>::max();
...
void KaurADijkstraAlgSTL::WorkerLoop(int thread_id) {
  int my_gen = 0;
...
void KaurADijkstraAlgSTL::Dispatch(Phase phase) {
  {
    std::scoped_lock<std::mutex> lock(mtx_);
    phase_ = phase;
...
bool KaurADijkstraAlgSTL::RunImpl() {
  const InType n = GetInput();
  const InType inf = std::numeric_limits<InType>::max();
...
bool KaurADijkstraAlgSTL::PostProcessingImpl() {
  {
    std::scoped_lock<std::mutex> lock(mtx_);
    phase_ = Phase::kStop;
    ++generation_;
