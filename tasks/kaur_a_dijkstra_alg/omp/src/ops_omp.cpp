...
KaurADijkstraAlgOMP::KaurADijkstraAlgOMP(const InType &in) {
  SetTypeOfTask(GetStaticTypeOfTask());
  GetInput() = in;
  GetOutput() = 0;
}

bool KaurADijkstraAlgOMP::ValidationImpl() {
  return GetInput() > 0;
}

bool KaurADijkstraAlgOMP::PreProcessingImpl() {
...
bool KaurADijkstraAlgOMP::RunImpl() {
  const InType n = GetInput();
  const InType inf = std::numeric_limits<InType>::max();
...
bool KaurADijkstraAlgOMP::PostProcessingImpl() {
  return true;
}
