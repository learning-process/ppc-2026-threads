...
void AddToBuffer(const double *a, std::size_t a_stride,
                 const double *b, std::size_t b_stride,
                 double *dst, std::size_t n, double b_coeff) {
  for (std::size_t i = 0; i < n; ++i) {
...
void NaiveMulBlocked(const double *a, std::size_t a_stride,
                     const double *b, std::size_t b_stride,
                     double *c, std::size_t c_stride, std::size_t n) {
...
void CombineQuadrants(const std::vector<double> &m1, const std::vector<double> &m2,
                      const std::vector<double> &m3, const std::vector<double> &m4,
                      const std::vector<double> &m5, const std::vector<double> &m6,
                      const std::vector<double> &m7,
                      double *c, std::size_t c_stride, std::size_t half) {
...
// forward declaration
void StrassenSeq(const double *a, std::size_t a_stride,
                 const double *b, std::size_t b_stride,
                 double *c, std::size_t c_stride, std::size_t n);
...
void StrassenSeq(const double *a, std::size_t a_stride,
  ...
  const double *a11 = a;           const double *a12 = a + half;
  const double *a21 = a + half * a_stride; const double *a22 = a21 + half;
  const double *b11 = b;           const double *b12 = b + half;
  const double *b21 = b + half * b_stride; const double *b22 = b21 + half;

  std::vector<double> lhs(half * half), rhs(half * half);
  std::vector<double> m1(half*half), m2(half*half), m3(half*half),
                      m4(half*half), m5(half*half), m6(half*half), m7(half*half);
...
// OMP только на верхнем уровне рекурсии
void StrassenTopOmp(const double *a, std::size_t a_stride,
                    const double *b, std::size_t b_stride,
                    double *c, std::size_t c_stride, std::size_t n) {
  if (n <= kCutoff) {
    NaiveMulBlocked(a, a_stride, b, b_stride, c, c_stride, n);
...
#pragma omp parallel default(none) \
    shared(m1, m2, m3, m4, m5, m6, m7, \
           a11, a12, a21, a22, b11, b12, b21, b22, \
           a_stride, b_stride, half)
  {
...
bool MuhammadkhonIStressenAlgOMP::RunImpl() {
  ...
  StrassenTopOmp(padded_a_.data(), padded_n_,
                 padded_b_.data(), padded_n_,
                 result_c_.data(), padded_n_,
                 padded_n_);
