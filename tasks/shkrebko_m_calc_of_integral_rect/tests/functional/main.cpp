...
const std::array<TestType, 10> kTestCases = {
    ...
    TestType{"Const_2D", MakeInType([](const std::vector<double> &) { return 1.0; }, {{0.0, 1.0}, {0.0, 2.0}}, 80),
             2.0},
    TestType{"Const_3D",
             MakeInType([](const std::vector<double> &) { return 1.0; }, {{0.0, 1.0}, {-1.0, 1.0}, {2.0, 5.0}}, 40),
             6.0},
    TestType{"Linear_1D", MakeInType([](const std::vector<double> &x) { return x[0]; }, {{0.0, 2.0}}, 200), 2.0},
    TestType{"Linear_2D",
             MakeInType([](const std::vector<double> &x) { return x[0] + x[1]; }, {{0.0, 1.0}, {0.0, 1.0}}, 80), 1.0},
    TestType{"Quad_1D", MakeInType([](const std::vector<double> &x) { return x[0] * x[0]; }, {{-1.0, 2.0}}, 200), 3.0},
    TestType{"Prod_2D",
             MakeInType([](const std::vector<double> &x) { return x[0] * x[1]; }, {{0.0, 2.0}, {1.0, 3.0}}, 100), 8.0},
    TestType{"Trig_sin_1D",
             MakeInType([](const std::vector<double> &x) { return std::sin(x[0]); }, {{0.0, std::numbers::pi}}, 200),
             2.0},
    TestType{"Exp_1D", MakeInType([](const std::vector<double> &x) { return std::exp(x[0]); }, {{0.0, 1.0}}, 200),
             std::numbers::e - 1.0},
