...
2. **Параллельная поразрядная сортировка:** `#pragma omp parallel for` для сортировки каждого блока.
3. **Параллельное слияние:** итеративное слияние блоков внутри процесса.

Таким образом, каждый MPI-процесс ведёт себя как независимая OpenMP-программа, сортирующая свою часть массива.

## 6. Детали реализации

- **Файлы:** `all/include/ops_all.hpp`, `all/src/ops_all.cpp`, класс `GolovanovDRadixMergeALL`.
...
Те же функциональные тесты, что для SEQ. Дополнительно проверяется, что результат одинаков на всех MPI-процессах после завершения сортировки.
...
| Конфигурация | Процессов | Потоков | Всего работников | Время, с (из лога) | Ускорение `S = T_seq / T_all` | Эффективность `Eff = S/workers` |
|--------------|-----------|---------|------------------|--------------------|------------------------------|--------------------------------|
...
| Конфигурация | Процессов | Потоков | Всего работников | Время, с (из лога) | Ускорение `S = T_seq / T_all` | Эффективность |
|--------------|-----------|---------|------------------|--------------------|------------------------------|---------------|
...
**Что получилось.** Гибридная версия ALL (MPI + OpenMP) успешно реализована и проходит все тесты корректности. На конфигурации **2 процесса × 4 потока** достигнуто ускорение **4.59×** в режиме `task_run` и **5.00×** в режиме `pipeline` — это **лучший результат среди всех реализаций** на 8 работниках.

**Норм ли реализация.** Да, реализация полностью корректна. MPI-обмены работают стабильно, OpenMP-сортировка внутри процессов — корректно. Гибридная схема показала себя эффективнее, чем использование только OpenMP на том же количестве работников (4.59× против 4.20×).

**Отличия от последовательной версии:**
- **В лучшую сторону:** значительное ускорение на всех конфигурациях с числом работников >1. Лучший результат — 5.00× на конфигурации 2×4.
- **В худшую сторону:** при 1 процессе × 1 потоке время сопоставимо с SEQ (незначительное замедление из-за MPI-инициализации).

**Отличия от чистых технологий (OpenMP, TBB, STL):**
- **Лучший результат** на 8 работниках (5.00× против 4.73× у STL).
- **Почему:** разделение данных между MPI-процессами уменьшает объём данных, которые нужно сливать внутри одного процесса. При конфигурации 2×4 каждый процесс сортирует ~500k элементов, а затем выполняется только один шаг MPI-слияния (вместо многоуровневого слияния в чистом OpenMP).
- **Обратная сторона:** MPI-обмены (Send/Recv) вносят дополнительные накладные расходы, которые могут перевесить выгоду при неудачном выборе конфигурации.
...
- Параллельное слияние через MPI-обмены — эффективно, но только если число процессов невелико (степень двойки для сбалансированного дерева).
- OpenMP внутри каждого процесса — ускорение локальной сортировки.

**Итог:** Гибридная версия ALL является **лучшей среди всех реализаций** на 8 работниках (ускорение 5.00× в режиме `pipeline`). Однако это достижение требует осторожной интерпретации: тесты проводились на **одном узле**, где MPI-обмены происходят через разделяемую память (shared memory), а не по сети. На реальном кластере с сетевыми задержками результат может быть хуже. Тем не менее, для одноузловых гибридных систем ALL показывает, что разумное сочетание MPI и OpenMP может превзойти чистые потоковые технологии.
