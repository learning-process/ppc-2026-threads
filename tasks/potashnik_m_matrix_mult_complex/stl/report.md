...
STL-реализация детерминирована: разбиение на диапазоны фиксировано формулой `chunk`, порядок слияния последователен. Для всех размеров матриц результаты побитово совпадают с SEQ.
...
**Накладные расходы create/join:** создание и join 4 потоков занимает 0.2–0.5 мс, что незначительно по сравнению с 44 мс вычислений.

**Сравнение с теоретическим максимумом:** идеальное ускорение на 4 потоках — `149 / 4 = 37.3 мс`. Реальное — 44 мс. Потери ~6.7 мс приходятся на слияние локальных карт.
