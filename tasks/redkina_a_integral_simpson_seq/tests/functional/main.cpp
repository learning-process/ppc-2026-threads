...
class RedkinaAIntegralSimpsonFuncTests
...
InputData MakeInput(std::function<double(const std::vector<double>&)> func,
                    std::vector<double> a,
                    std::vector<double> b,
                    std::vector<int> n) {
  return InputData{
...
const std::array<TestType, 20> kTestCases = {{
    // 1D: константа
    std::make_tuple(1,
                    MakeInput([](const std::vector<double>&) { return 1.0; },
    ...
    // 1D: линейная
    std::make_tuple(2,
                    MakeInput([](const std::vector<double>& x) { return x[0]; },
                              std::vector<double>{0.0},
                              std::vector<double>{1.0},
                              std::vector<int>{2}),
                    0.5),
    ...
    // 1D: квадратичная
    std::make_tuple(3,
                    MakeInput([](const std::vector<double>& x) { return x[0] * x[0]; },
                              std::vector<double>{0.0},
                              std::vector<double>{1.0},
                              std::vector<int>{2}),
                    1.0 / 3.0),
    ...
    // 1D: sin(x)
    std::make_tuple(6,
                    MakeInput([](const std::vector<double>& x) { return std::sin(x[0]); },
                              std::vector<double>{0.0},
    ...
    // 1D: exp(x)
    std::make_tuple(7,
                    MakeInput([](const std::vector<double>& x) { return std::exp(x[0]); },
                              std::vector<double>{0.0},
                              std::vector<double>{1.0},
                              std::vector<int>{200}),
                    kE - 1.0),
    ...
    // 2D: x*y
    std::make_tuple(9,
                    MakeInput([](const std::vector<double>& x) { return x[0] * x[1]; },
                              std::vector<double>{0.0, 0.0},
                              std::vector<double>{1.0, 1.0},
                              std::vector<int>{2, 2}),
                    0.25),
    ...
    // 2D: x*sin(y)
    std::make_tuple(15,
                    MakeInput([](const std::vector<double>& x) { return x[0] * std::sin(x[1]); },
                              std::vector<double>{0.0, 0.0},
                              std::vector<double>{1.0, kPi},
                              std::vector<int>{200, 200}),
                    1.0),
    ...
    // 3D: x*y*z
    std::make_tuple(17,
                    MakeInput([](const std::vector<double>& x) { return x[0] * x[1] * x[2]; },
                              std::vector<double>{0.0, 0.0, 0.0},
                              std::vector<double>{1.0, 1.0, 1.0},
                              std::vector<int>{2, 2, 2}),
                    0.125),
    ...
    std::make_tuple(20,
                    MakeInput([](const std::vector<double>& x) { return std::sin(x[0]) * std::cos(x[1]) * std::exp(x[2]); },
                              std::vector<double>{0.0, 0.0, 0.0},
                              std::vector<double>{kPi, kPi, 1.0},
                              std::vector<int>{100, 100, 100}),
                    0.0)
}};

const auto kTestTasksList = ppc::util::AddFuncTask<RedkinaAIntegralSimpsonSEQ, InType>(
    kTestCases, PPC_SETTINGS_redkina_a_integral_simpson_seq);
