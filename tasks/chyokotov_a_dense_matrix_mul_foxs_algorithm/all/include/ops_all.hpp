...
class ChyokotovADenseMatMulFoxAlgorithmALL : public BaseTask {
 ...
 private:
  ...
  int CalcPaddedSize(int n, int q);
  void PadMatrix(const std::vector<double> &src, std::vector<double> &dst, int original_n, int padded_n);

  void Multiply(const std::vector<double> &a_block, const std::vector<double> &b_block, std::vector<double> &c_block,
                int block_size);
