...
Нужно вычислить произведение $C = A \cdot B$ для двух разреженных **комплексных** матриц в формате **CCS** (Compressed Column Storage).
...
В технологии ALL основное умножение вызывается через `BuildProductMatrix`, а в `RunImpl` дополнительно демонстрируются MPI, OpenMP, `std::thread` и TBB.
...
После умножения в `RunImpl` выполняются учебные фрагменты с потоками (не влияют на математику $C = A \cdot B$, но влияют на время perf).
...
После умножения — учебные фрагменты (OpenMP на `rank == 0`, цикл `std::thread`, `tbb::parallel_for`), затем `MPI_Barrier`. Они **увеличивают** время perf по сравнению с чистым SEQ.
...
**`task_run`**, 1 MPI-процесс × $p$ потоков, $T_{seq} = 0.00001390$ с.
...
_Комментарий:_ ALL **медленнее** чистого SEQ, потому что в `RunImpl` есть лишняя работа (демо-потоки и барьер MPI). Чем больше `PPC_NUM_THREADS`, тем дольше идут эти дополнительные циклы.

## 9. Выводы

ALL — «сборная» версия для курса: MPI + несколько API. Для оценки скорости самого умножения смотрите отчёты SEQ, OMP и TBB. ALL показывает, как связать разные технологии в одной программе.
