...
Вариант **STL** в этой задаче — это класс `SabutaySparseComplexCcsMultSTL` в каркасе курса. Само умножение матриц выполняется **последовательно**, тем же кодом, что и SEQ. Отдельные потоки `std::thread` показаны в технологии ALL (после умножения), а не в STL-файле.
...
Потоки `std::thread` в этой задаче используются в классе **ALL** (`RunImpl`: создаётся `num_threads` потоков, каждый увеличивает счётчик, затем `join`). В `stl/src/ops_stl.cpp` ручного разбиения диапазонов и `join` для SpMM **нет** — это важно указать в отчёте честно.
...
**`task_run`**, $T_{seq} = 0.00001390$ с. Значения $S > 1$ — погрешность замера, а не реальное ускорение (код однопоточный).
...
_Комментарий:_ время около $10^{-5}$ с и почти не зависит от $p$ — так и должно быть, потому что код однопоточный. Небольшие отличия от SEQ — погрешность замера, а не ускорение.

## 9. Выводы

STL в проекте — отдельный класс задачи с тем же последовательным умножением, что SEQ. Для ускорения по потокам в этой работе используются OMP и TBB.
