...
PetersonGrahamScannerSeq::PetersonGrahamScannerSeq(const InputValue &in) {
  SetTypeOfTask(GetStaticTypeOfTask());
  GetInput() = in;
  GetOutput() = 0;
}

void PetersonGrahamScannerSeq::LoadPoints(const PointSet &points) {
  input_points_ = points;
  external_data_provided_ = true;
}

PointSet PetersonGrahamScannerSeq::GetConvexHull() const {
  return hull_points_;
}

bool PetersonGrahamScannerSeq::ValidationImpl() {
  return GetInput() >= 0;
}

bool PetersonGrahamScannerSeq::PreProcessingImpl() {
  hull_points_.clear();
...
bool PetersonGrahamScannerSeq::RunImpl() {
  hull_points_.clear();
...
bool PetersonGrahamScannerSeq::PostProcessingImpl() {
  GetOutput() = GetInput();
  return true;
}

double PetersonGrahamScannerSeq::ComputeOrientation(const Point2D &origin, const Point2D &a, const Point2D &b) {
  return CalculateOrientation(origin, a, b);
}

double PetersonGrahamScannerSeq::ComputeDistanceSq(const Point2D &p1, const Point2D &p2) {
  return CalculateSquaredDistance(p1, p2);
}

bool PetersonGrahamScannerSeq::AreAllPointsIdentical(const PointSet &points) {
  if (points.empty()) {
    return true;
  }
...
std::size_t PetersonGrahamScannerSeq::FindLowestPoint(const PointSet &points) {
  std::size_t lowest = 0;
...
void PetersonGrahamScannerSeq::SortByAngle(PointSet &points) {
  if (points.size() < 2) {
    return;
  }
