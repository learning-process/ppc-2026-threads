...
void ExecutePipeline(const std::shared_ptr<PetersonGrahamScannerSeq> &task) {
  task->Validation();
  task->PreProcessing();
  task->Run();
  task->PostProcessing();
...
TEST(PetersonGrahamScannerSeq, TwoDistinctPoints) {
...
TEST(PetersonGrahamScannerSeq, CollinearPoints) {
...
TEST(PetersonGrahamScannerSeq, TrianglePoints) {
...
TEST(PetersonGrahamScannerSeq, SquarePoints) {
...
TEST(PetersonGrahamScannerSeq, NegativeInput) {
  auto task = std::make_shared<PetersonGrahamScannerSeq>(-1);
