...
PetersonGrahamScannerALL::PetersonGrahamScannerALL(const InputValue &in) {
  SetTypeOfTask(GetStaticTypeOfTask());
  GetInput() = in;
  GetOutput() = 0;
}

void PetersonGrahamScannerALL::LoadPoints(const PointSet &points) {
  input_points_ = points;
  external_data_provided_ = true;
}

PointSet PetersonGrahamScannerALL::GetConvexHull() const {
  return hull_points_;
}

bool PetersonGrahamScannerALL::ValidationImpl() {
  return GetInput() >= 0;
}

bool PetersonGrahamScannerALL::PreProcessingImpl() {
  hull_points_.clear();
...
bool PetersonGrahamScannerALL::AreAllPointsIdentical(const PointSet &points) {
  if (points.empty()) {
    return true;
  }
...
std::size_t PetersonGrahamScannerALL::FindLowestPointParallel(const PointSet &points) {
...
void PetersonGrahamScannerALL::SortPointsByAngleParallel(PointSet &points) {
  if (points.size() < 2) {
    return;
  }
...
bool PetersonGrahamScannerALL::RunImpl() {
  hull_points_.clear();
...
bool PetersonGrahamScannerALL::PostProcessingImpl() {
  GetOutput() = static_cast<int>(hull_points_.size());
  return true;
}

double PetersonGrahamScannerALL::ComputeOrientation(const Point2D &origin, const Point2D &a, const Point2D &b) {
  return CalculateOrientation(origin, a, b);
}

double PetersonGrahamScannerALL::ComputeDistanceSq(const Point2D &p1, const Point2D &p2) {
