...
void AddBaseOffsets(OutType &output, int cols, int num_threads, const std::vector<int> &row_starts,
                    const std::vector<int> &base) {
#pragma omp parallel for default(none) shared(output, cols, num_threads, row_starts, base) schedule(static)
  for (int thread_idx = 0; thread_idx < num_threads; thread_idx++) {
...
#pragma omp parallel for default(none) shared(first_pos, thread_first, max_label, num_threads) schedule(static)
