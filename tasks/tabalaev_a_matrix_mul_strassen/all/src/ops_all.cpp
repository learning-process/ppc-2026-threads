...
bool TabalaevAMatrixMulStrassenALL::RunImpl() {
  ...
  if (rank == 0) {
    dims[0] = static_cast<unsigned long long>(a_rows_);
    dims[1] = static_cast<unsigned long long>(b_cols_);
    dims[2] = static_cast<unsigned long long>(padded_n_);
  }
  ...
  if (rank == 0) {
    if (size == 1 || padded_n_ <= kBaseCaseSize) {
      ...
      unsigned long long terminate_signal = 0;
      for (int dest = 1; dest < size; ++dest) {
        MPI_Send(&terminate_signal, 1, MPI_UNSIGNED_LONG_LONG, dest, 0, MPI_COMM_WORLD);
      }
    } else {
      ...
      for (int dest = 1; dest < size && tasks_sent < num_tasks; ++dest) {
        unsigned long long h_msg = static_cast<unsigned long long>(h);
        MPI_Send(&h_msg, 1, MPI_UNSIGNED_LONG_LONG, dest, 0, MPI_COMM_WORLD);
        MPI_Send(&tasks_sent, 1, MPI_INT, dest, 1, MPI_COMM_WORLD);
        MPI_Send(task_a[tasks_sent].data(), h * h, MPI_DOUBLE, dest, 2, MPI_COMM_WORLD);
        MPI_Send(task_b[tasks_sent].data(), h * h, MPI_DOUBLE, dest, 3, MPI_COMM_WORLD);
        tasks_sent++;
      }
      ...
      while (tasks_completed < num_tasks) {
        ...
        MPI_Recv(p[task_id].data(), h * h, MPI_DOUBLE, worker, 5, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        tasks_completed++;

        if (tasks_sent < num_tasks) {
          unsigned long long h_msg = static_cast<unsigned long long>(h);
          MPI_Send(&h_msg, 1, MPI_UNSIGNED_LONG_LONG, worker, 0, MPI_COMM_WORLD);
          MPI_Send(&tasks_sent, 1, MPI_INT, worker, 1, MPI_COMM_WORLD);
          MPI_Send(task_a[tasks_sent].data(), h * h, MPI_DOUBLE, worker, 2, MPI_COMM_WORLD);
          MPI_Send(task_b[tasks_sent].data(), h * h, MPI_DOUBLE, worker, 3, MPI_COMM_WORLD);
          tasks_sent++;
        }
      }
...
#pragma omp parallel for default(none) shared(out, result_c, a_rows, b_cols, padded_n)
    ...
    while (true) {
      unsigned long long h_msg = 0;
      MPI_Status status;
      MPI_Recv(&h_msg, 1, MPI_UNSIGNED_LONG_LONG, 0, 0, MPI_COMM_WORLD, &status);
      ...
      MPI_Recv(a_sub.data(), h * h, MPI_DOUBLE, 0, 2, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      MPI_Recv(b_sub.data(), h * h, MPI_DOUBLE, 0, 3, MPI_COMM_WORLD, MPI_STATUS_IGNORE);

      std::vector<double> res = StrassenMultiply(a_sub, b_sub, h);

      MPI_Send(&task_id, 1, MPI_INT, 0, 4, MPI_COMM_WORLD);
      MPI_Send(res.data(), h * h, MPI_DOUBLE, 0, 5, MPI_COMM_WORLD);
    }
  }
  ...
  MPI_Bcast(out.data(), out.size(), MPI_DOUBLE, 0, MPI_COMM_WORLD);
