...
MatrixCRS DenseToCRS(const DenseMatrix &m, double eps = 1e-12) {
  ...
  r.rows = rows;
  r.cols = cols;
  r.row_ptr.resize(static_cast<std::size_t>(rows) + 1);
  r.row_ptr[0] = 0;
...
class ErmakovARunPerfTestSparMatMultOMP : public ppc::util::BaseRunPerfTests<InType, OutType> {
 protected:
  ...
  bool CheckTestOutputData(OutType &output_data) final {
    ...
    if (output_data.row_ptr.size() != static_cast<std::size_t>(output_data.rows) + 1) {
      return false;
    }
    ...
    if (static_cast<std::size_t>(output_data.row_ptr.back()) != output_data.values.size()) {
      return false;
    }
...
TEST_P(ErmakovARunPerfTestSparMatMultOMP, RunPerfModes) {
...
const auto kAllPerfTasks =
    ppc::util::MakeAllPerfTasks<InType, ErmakovASparMatMultOMP>(PPC_SETTINGS_ermakov_a_spar_mat_mult_omp);

const auto kGtestValues = ppc::util::TupleToGTestValues(kAllPerfTasks);

const auto kPerfTestName = ErmakovARunPerfTestSparMatMultOMP::CustomPerfTestName;

INSTANTIATE_TEST_SUITE_P(RunModeTests, ErmakovARunPerfTestSparMatMultOMP, kGtestValues, kPerfTestName);
