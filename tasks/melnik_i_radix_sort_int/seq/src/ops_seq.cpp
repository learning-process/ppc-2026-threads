...
void MelnikIRadixSortIntSEQ::CountingSort(OutType &data, int exp) {
  ...
  for (int i = 0; i < n; i++) {
    int digit = (data[i] / exp) & kDigitMask;
    count[digit]++;
  }
...
void MelnikIRadixSortIntSEQ::RadixSort(OutType &data) {
  ...
  if (max_val <= 0) {
    for (int exp = 1; min_val / exp > 0 || (-max_val) / exp > 0; exp <<= kBitsPerDigit) {
      CountingSort(data, exp);
    }
  ...
  if (!negatives.empty()) {
    ...
    int neg_min = *std::min_element(negatives.begin(), negatives.end());
    for (int exp = 1; neg_max / exp > 0 || (-neg_min) / exp > 0; exp <<= kBitsPerDigit) {
