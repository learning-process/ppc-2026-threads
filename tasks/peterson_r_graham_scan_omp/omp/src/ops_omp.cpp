...
bool PetersonRGrahamScanOMP::PreProcessingImpl() {
  ...
  if (!is_custom_input_) {
    ...
    for (int i = 0; i < point_count; ++i) {
      ...
      input_points_.emplace_back(Point{.x = std::cos(angle), .y = std::sin(angle)});
    }
  } else {
...
bool PetersonRGrahamScanOMP::RunImpl() {
  ...
  std::iter_swap(input_points_.begin(), input_points_.begin() + static_cast<std::ptrdiff_t>(pivot_idx));
