...
ZhurinIGaussKernelSTL::ZhurinIGaussKernelSTL(const InType &in) {
  SetTypeOfTask(GetStaticTypeOfTask());
  GetInput() = in;
  GetOutput() = OutType{};
}

bool ZhurinIGaussKernelSTL::ValidationImpl() {
...
bool ZhurinIGaussKernelSTL::PreProcessingImpl() {
  const auto &in = GetInput();
  width_ = std::get<0>(in);
...
bool ZhurinIGaussKernelSTL::RunImpl() {
  int w = width_;
  int h = height_;
  int np = num_parts_;
  int base_width = w / np;
...
bool ZhurinIGaussKernelSTL::PostProcessingImpl() {
