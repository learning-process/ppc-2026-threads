...
ZhurinIGaussKernelTBB::ZhurinIGaussKernelTBB(const InType &in) {
  SetTypeOfTask(GetStaticTypeOfTask());
  GetInput() = in;
  GetOutput() = OutType{};
}

bool ZhurinIGaussKernelTBB::ValidationImpl() {
...
bool ZhurinIGaussKernelTBB::PreProcessingImpl() {
  const auto &in = GetInput();
  width_ = std::get<0>(in);
...
bool ZhurinIGaussKernelTBB::RunImpl() {
  int w = width_;
  int h = height_;
  int np = num_parts_;
  int base_width = w / np;
...
bool ZhurinIGaussKernelTBB::PostProcessingImpl() {
