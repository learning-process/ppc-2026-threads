...
class KotelnikovaARunFuncTestsSEQ : public ppc::util::BaseRunFuncTests<InType, OutType, TestType> {
 ...
 protected:
  void SetUp() override {
    ...
    switch (test_case) {
      case 1: {
        SparseMatrixCCS A = CreateMatrix(3, 3, {1.5, 4.2, 3.7, 2.8, 5.1}, {0, 2, 1, 0, 2}, {0, 2, 3, 5});

        SparseMatrixCCS B = CreateMatrix(3, 3, {1.2, 2.3, 3.4}, {0, 1, 2}, {0, 1, 2, 3});
      ...
      case 2: {
        SparseMatrixCCS A = CreateMatrix(2, 3, {1.2, 2.5, 3.7}, {0, 0, 1}, {0, 1, 2, 3});

        SparseMatrixCCS B = CreateMatrix(3, 2, {1.1, 3.3, 2.2}, {0, 2, 1}, {0, 2, 3});
      ...
      case 3: {
        SparseMatrixCCS A = CreateMatrix(2, 2, {2.5}, {0}, {0, 1, 1});

        SparseMatrixCCS B = CreateMatrix(2, 2, {3.7}, {1}, {0, 1, 1});
      ...
      case 4: {
        SparseMatrixCCS A = CreateMatrix(3, 3, {1.1, 4.2, 7.3, 2.4, 5.5, 8.6, 3.7, 6.8, 9.9},
                                         {0, 1, 2, 0, 1, 2, 0, 1, 2}, {0, 3, 6, 9});

        SparseMatrixCCS B = CreateMatrix(3, 3, {1.0, 1.0, 1.0}, {0, 1, 2}, {0, 1, 2, 3});
      ...
      case 5: {
        SparseMatrixCCS A = CreateMatrix(4, 4, {1.5, 2.5, 3.5, 4.5}, {0, 1, 2, 3}, {0, 1, 2, 3, 4});

        SparseMatrixCCS B = CreateMatrix(4, 4, {5.5, 6.5, 7.5, 8.5}, {0, 1, 2, 3}, {0, 1, 2, 3, 4});
