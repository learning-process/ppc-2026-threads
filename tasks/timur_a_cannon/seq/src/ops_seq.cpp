...
bool TimurACannonMatrixMultiplication::RunImpl() {
  ...
  int n = static_cast<int>(mat_a.size());
  int grid_sz = n / b_size;

  using Matrix = std::vector<std::vector<double>>;
  using BlockGrid = std::vector<std::vector<Matrix>>;

  BlockGrid blocks_a(grid_sz, std::vector<Matrix>(grid_sz, Matrix(b_size, std::vector<double>(b_size))));
  BlockGrid blocks_b(grid_sz, std::vector<Matrix>(grid_sz, Matrix(b_size, std::vector<double>(b_size))));
  BlockGrid blocks_c(grid_sz, std::vector<Matrix>(grid_sz, Matrix(b_size, std::vector<double>(b_size, 0.0))));
  ...
  for (int step = 0; step < grid_sz; ++step) {
    for (int i = 0; i < grid_sz; ++i) {
      for (int j = 0; j < grid_sz; ++j) {
        BlockMultiplyAccumulate(blocks_a[i][j], blocks_b[i][j], blocks_c[i][j], b_size);
      }
    }
    if (step < grid_sz - 1) {
      RotateBlocksA(blocks_a, grid_sz);
      RotateBlocksB(blocks_b, grid_sz);
    }
  }
