...
bool KondrashovaVTaskOMP::ValidationImpl() { return true; }
...
void ScanStripe(int row_start, int row_end, int width, int label_offset,
  ...
  for (int ii = row_start; ii < row_end; ++ii) {
    for (int jj = 0; jj < width; ++jj) {
      ...
      int left_label =
          GetNeighborLabel(ii, jj, 0, -1, row_start, row_end, width, image, local_labels);
      int top_label =
          GetNeighborLabel(ii, jj, -1, 0, row_start, row_end, width, image, local_labels);
...
void MergeHorizontal(int height, int width, const std::vector<int> &local_labels,
...
void MergeBoundaries(int height, int width, int num_threads,
                     const std::vector<int> &local_labels,
                     std::vector<int> &parent, std::vector<int> &rnk) {
  for (int tid = 1; tid < num_threads; ++tid) {
    const int boundary_row = (tid * height) / num_threads;
    if (boundary_row >= height) continue;
    for (int jj = 0; jj < width; ++jj) {
      auto idx =
          static_cast<size_t>(boundary_row) * static_cast<size_t>(width) +
          static_cast<size_t>(jj);
      auto tidx =
          static_cast<size_t>(boundary_row - 1) * static_cast<size_t>(width) +
          static_cast<size_t>(jj);
...
#pragma omp parallel num_threads(num_threads) \
    default(none) shared(local_labels, max_labels_per_thread)
  {
  ...
  GetOutput().count =
      Relabel(total, local_labels, parent, relabel_map, labels_1d_);
