...
bool KondrashovaVTaskALL::ValidationImpl() { return true; }
...
int Find(std::vector<int> &parent, int xx) {
  while (parent[static_cast<size_t>(xx)] != xx) {
    parent[static_cast<size_t>(xx)] =
        parent[static_cast<size_t>(parent[static_cast<size_t>(xx)])];
...
int GetNeighborLabel(int ii, int jj, int di, int dj, int row_start,
                     int row_end, int width,
                     const std::vector<uint8_t> &image,
                     const std::vector<int> &local_labels) {
  int ni = ii + di;
  int nj = jj + dj;
  if (ni < row_start || ni >= row_end || nj < 0 || nj >= width) return 0;
  auto nidx = (static_cast<size_t>(ni) * static_cast<size_t>(width)) +
              static_cast<size_t>(nj);
  if (image[nidx] == 0) return local_labels[nidx];
  return 0;
}

void ScanStripe(int row_start, int row_end, int width, int label_offset,
                const std::vector<uint8_t> &image,
                std::vector<int> &local_labels) {
  int current_label = label_offset;
  for (int ii = row_start; ii < row_end; ++ii) {
    for (int jj = 0; jj < width; ++jj) {
      auto idx = (static_cast<size_t>(ii) * static_cast<size_t>(width)) +
                 static_cast<size_t>(jj);
      if (image[idx] != 0) continue;

      int left_label = GetNeighborLabel(ii, jj, 0, -1, row_start, row_end,
                                        width, image, local_labels);
      int top_label = GetNeighborLabel(ii, jj, -1, 0, row_start, row_end,
                                       width, image, local_labels);
...
void MergeHorizontal(int height, int width,
                     const std::vector<int> &local_labels,
  ...
  for (int ii = 0; ii < height; ++ii) {
    for (int jj = 1; jj < width; ++jj) {
      auto idx = (static_cast<size_t>(ii) * static_cast<size_t>(width)) +
                 static_cast<size_t>(jj);
      auto lidx = (static_cast<size_t>(ii) * static_cast<size_t>(width)) +
                  static_cast<size_t>(jj - 1);
      if (local_labels[idx] != 0 && local_labels[lidx] != 0 &&
          local_labels[idx] != local_labels[lidx]) {
...
void MergeVertical(int height, int width, const std::vector<int> &local_labels,
                   std::vector<int> &parent, std::vector<int> &rnk) {
  for (int ii = 1; ii < height; ++ii) {
    for (int jj = 0; jj < width; ++jj) {
      auto idx = (static_cast<size_t>(ii) * static_cast<size_t>(width)) +
                 static_cast<size_t>(jj);
      auto tidx = (static_cast<size_t>(ii - 1) * static_cast<size_t>(width)) +
                  static_cast<size_t>(jj);
      if (local_labels[idx] != 0 && local_labels[tidx] != 0 &&
          local_labels[idx] != local_labels[tidx]) {
...
void MergeBoundaries(int row_start, int row_end, int width, int num_threads,
                     const std::vector<int> &local_labels,
                     std::vector<int> &parent, std::vector<int> &rnk) {
  ...
  for (int tid = 1; tid < num_threads; ++tid) {
    ...
    for (int jj = 0; jj < width; ++jj) {
      auto idx =
          (static_cast<size_t>(boundary_row) * static_cast<size_t>(width)) +
          static_cast<size_t>(jj);
      auto tidx =
          (static_cast<size_t>(boundary_row - 1) *
...
int Relabel(int total, const std::vector<int> &local_labels,
            std::vector<int> &parent, std::vector<int> &relabel_map,
            std::vector<int> &labels_1d) {
  int count = 0;
  for (int ii = 0; ii < total; ++ii) {
    auto idx = static_cast<size_t>(ii);
    if (local_labels[idx] == 0) continue;
...
void MergeMPIBoundaries(int width, int rank, int world_size,
    ...
    if (local_row_start < local_row_end) {
      for (int jj = 0; jj < width; ++jj) {
        send_row[static_cast<size_t>(jj)] =
            local_labels[static_cast<size_t>(local_row_start) *
                             static_cast<size_t>(width) +
                         static_cast<size_t>(jj)];
      }
    }

    MPI_Sendrecv(send_row.data(), width, MPI_INT, rank - 1, 0,
                 recv_row.data(), width, MPI_INT, rank - 1, 1,
  ...
  if (rank < world_size - 1) {
    ...
    if (local_row_start < local_row_end) {
      ...
      for (int jj = 0; jj < width; ++jj) {
        send_row[static_cast<size_t>(jj)] =
            local_labels[static_cast<size_t>(last_row) *
                             static_cast<size_t>(width) +
                         static_cast<size_t>(jj)];
      }
    }

    MPI_Sendrecv(send_row.data(), width, MPI_INT, rank + 1, 1,
                 recv_row.data(), width, MPI_INT, rank + 1, 0,
                 MPI_COMM_WORLD, MPI_STATUS_IGNORE);
...
#pragma omp parallel num_threads(num_threads) default(none)        \
    shared(local_labels, width, image, num_threads)                \
    firstprivate(mpi_row_start, max_labels_per_thread)
  {
    const int tid = omp_get_thread_num();
    const int omp_row_start =
        mpi_row_start + (tid * mpi_rows) / num_threads;
    const int omp_row_end =
        mpi_row_start + ((tid + 1) * mpi_rows) / num_threads;
    const int label_offset =
        (rank_ * num_threads * max_labels_per_thread) +
        (tid * max_labels_per_thread);
    ScanStripe(omp_row_start, omp_row_end, width, label_offset, image,
               local_labels);
  }

  const int global_max_labels =
      (world_size_ * num_threads * max_labels_per_thread) + 1;
  ...
  MergeBoundaries(mpi_row_start, mpi_row_end, width_, num_threads, local_labels,
                  parent, rnk);

  MergeMPIBoundaries(width_, rank_, world_size_, mpi_row_start, mpi_row_end,
                     local_labels, parent, rnk);

  MPI_Barrier(MPI_COMM_WORLD);

  std::vector<int> all_local_labels(static_cast<size_t>(total), 0);
  MPI_Allreduce(local_labels.data(), all_local_labels.data(), total,
  ...
  if (rank_ == 0) {
    ...
    MergeHorizontal(height_, width_, all_local_labels, global_parent,
                    global_rnk);
    ...
    GetOutput().count = Relabel(total, all_local_labels, global_parent,
                                relabel_map, labels_1d_);
  }
  ...
  if (rank_ == 0) {
    ...
    for (int ii = 0; ii < height_; ++ii) {
      for (int jj = 0; jj < width_; ++jj) {
        auto idx = (static_cast<size_t>(ii) * static_cast<size_t>(width_)) +
                   static_cast<size_t>(jj);
