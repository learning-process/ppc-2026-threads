...
## 1. Контекст
Гибридная версия (ALL) сочетает межпроцессный параллелизм (MPI) для распределения данных между узлами и внутрипроцессный параллелизм (OpenMP) для ускорения вычислений внутри каждого процесса. Цель — оценить эффективность двухуровневой декомпозиции для задачи поразрядной сортировки и понять, когда гибридный подход оправдан по сравнению с «чистыми» backend-ами (OMP, TBB, STL).
...
**Роли рангов:**
- `rank == 0` (мастер): вычисляет размеры чанков, рассылает данные, собирает и сливает результаты, рассылает финальный ответ.
...
| `MPI_Scatter` (для `recv_counts`) | Рассылка размеров чанков всем процессам | Каждый процесс должен знать, сколько данных ему придёт |
| `MPI_Scatterv` | Рассылка данных переменного размера | Чанки могут отличаться на 1 элемент при `n % num_processes != 0` |
| `MPI_Gatherv` | Сбор отсортированных чанков на `rank 0` | Для последующего слияния нужен полный массив на мастере |
| `MPI_Bcast` (размер + данные) | Рассылка финального результата всем процессам | Требуется для валидации в тестовом фреймворке курса |
...
- Явные `MPI_Barrier` не используются — коллективные операции (`MPI_Scatterv`, `MPI_Gatherv`, `MPI_Bcast`) содержат **неявные барьеры**.
- Последовательность вызовов гарантирует порядок: ни один процесс не начнёт `MPI_Gatherv`, пока все не завершат локальную сортировку.

## 5. Внутрипроцессная схема (уровень OpenMP)

**Выбранная технология:** OpenMP (`#pragma omp parallel for`) для параллельной обработки элементов внутри локального чанка каждого процесса.

**Почему OpenMP, а не std::thread или TBB внутри процесса:**
- Инфраструктура курса уже экспортирует `PPC_NUM_THREADS` как `OMP_NUM_THREADS`, что обеспечивает единообразие управления потоками.
- Директивы OpenMP позволяют распараллелить регулярные циклы с минимальными изменениями кода.
- `default(none)` требует явного указания атрибутов переменных, что повышает безопасность и соответствует требованиям `clang-tidy`.
...
|----------------|----------|----------------|
| `parallel for` | Параллельное выполнение итераций цикла между потоками | Регулярный доступ к элементам массива, независимые итерации |
| `schedule(static)` | Статическое разбиение итераций на равные чанки | Минимальный оверхед для циклов с равномерной нагрузкой |
| `default(none)` | Запрещает неявное определение атрибутов переменных | Требование безопасности и clang-tidy: явно указываем `shared`/`private` |
| `shared(temp, local_counts, n, shift)` | Эти переменные разделяются между потоками | Потоки читают `temp`, пишут в разные строки `local_counts` |
...
**Оптимальная конфигурация:** `2 ranks × 2 threads` (3.52×, 88%) или `2 ranks × 4 threads` (4.41×) для данной задачи на 6-ядерном процессоре.