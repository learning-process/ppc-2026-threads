...
STL-версия (ручное управление потоками через `std::thread`) реализована для оценки низкоуровневого подхода к распараллеливанию. В отличие от высокоуровневых технологий, здесь студент самостоятельно управляет созданием потоков, разбиением диапазонов, объединением результатов и синхронизацией. Это позволяет глубже понять накладные расходы на создание потоков и цену ручного управления параллелизмом.

**Базовый эталон:** `seq/report.md` (SEQ, время ~149 мс на матрицах 11000×11000 с 2 ненулевыми элементами на строку).
...
Двойной вложенный цикл является вычислительным ядром. Асимптотика по времени: O(`nnz_A` · `nnz_B`). В STL-версии внешний цикл распределяется между потоками вручную.
...
Каждый поток получает `tid`, диапазон `[start, end)`, ссылки на `matr_a`, `matr_b` и `local_maps`. Функция-воркер `Worker` выполняет основную работу и накапливает результаты в `local_maps[tid]`.
...
Для всех 10 размеров матриц и для `num_threads = 1, 2, 4` результаты побитово совпадают с SEQ. Это подтверждает корректность разбиения и отсутствие гонок.
...
Идеальное ускорение на 4 потоках: `149 / 4 = 37.3 мс`. Реальное: 44 мс. Потери `~6.7 мс` (около 15% от реального времени) приходятся на объединение локальных карт и борьбу за аллокатор.
...
STL-версия демонстрирует **хорошее ускорение** (3.39× на 4 потоках) с **высокой эффективностью** (84.8%). Основным узким местом является не модель распараллеливания, а внутреннее устройство `std::map`.
