...
Количество потоков выбирается как `min(task_count, hardware_concurrency)`; если `hardware_concurrency()` вернул `0`,
используется fallback `2` ([`stl/src/ops_stl.cpp`](src/ops_stl.cpp#L18)). `RunInThreads` распределяет задачи циклически:
...
|---:|---:|---:|---:|---|
| 12 | 0.0047718214 s | 1.221 | 0.102 | `std::thread::hardware_concurrency()`, `nproc=12`, `TaskRun` |
| 12 | 0.0055675704 s | 1.239 | 0.103 | `pipeline`, `PPC_NUM_THREADS=1`, фактически auto-threads |
| 12 | 0.0081357990 s | 0.848 | 0.071 | `pipeline`, `PPC_NUM_THREADS=4`, фактически auto-threads |

## Выводы

По коду STL-версия имеет те же фазы, что TBB: независимая сортировка блоков и независимое слияние. На `N=100000`
получено `0.0047718214 s`, speedup `1.221`; efficiency низкая (`0.102`), потому что реализация создает до 12 рабочих
потоков по `hardware_concurrency()`, а объем работы мал для такого числа потоков.
