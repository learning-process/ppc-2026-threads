...
что индексы столбцов в итоговой строке будут автоматически отсортированы, что является требованием формата CRS.
*   **Синхронизация**: явные барьеры не требуются, так как потоки пишут в непересекающиеся
области памяти (разные элементы внешних векторов). Финальная сборка CRS структуры выполняется последовательно после завершения параллельной секции.
