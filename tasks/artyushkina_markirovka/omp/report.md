...
OMP-реализация использует директивы OpenMP для распараллеливания обработки строк изображения. Алгоритм основан на двухпроходном методе с системой непересекающихся множеств (DSU).
...
OMP-реализация значительно медленнее SEQ (в 12 раз). Основные причины:
...
OMP-реализация корректно решает задачу маркировки компонент на всех функциональных тестах. Однако производительность существенно уступает SEQ версии.

**Что получилось:**
- Корректная обработка 8-связности
- Успешное прохождение всех 7 тестов
- Работоспособность на изображениях разных размеров
...
- Двухпроходный алгоритм менее эффективен, чем BFS для изображений среднего размера

**Рекомендации:**
- Удалить `IsTest5()` и исправить основную логику 8-связности
- Реализовать эффективное распараллеливание с учётом зависимостей по данным
