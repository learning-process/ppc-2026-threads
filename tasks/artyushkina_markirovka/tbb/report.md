...
TBB-реализация использует библиотеку Intel oneTBB для задачно-ориентированного распараллеливания. Алгоритм основан на двухпроходном методе с системой непересекающихся множеств (DSU) и синхронизацией через `tbb::spin_mutex`.
...
- `UnionLabels(int label1, int label2)` — объединение множеств с мьютексом
...
**Перенумерация (последовательная):**
```cpp
...
TBB-реализация показывает наилучший результат среди параллельных версий (0.018 с). Тем не менее, она всё ещё медленнее SEQ (в 3.5 раза). Основные причины:
- Двухпроходный алгоритм требует двух обходов изображения
- Накладные расходы на синхронизацию через мьютекс
- Последовательная перенумерация (`NormalizeLabelsTbb`) является узким местом

## 8. Выводы

TBB-реализация является наиболее проработанной среди параллельных версий. Параллельные этапы (инициализация, объединение, разрешение корней) демонстрируют эффективность TBB-планировщика.
...
- Мьютекс защищает операции Union, создавая точку синхронизации
- Нет ускорения относительно SEQ на изображении 1000x1000

**Рекомендации:**
- Параллелизовать этап `NormalizeLabelsTbb`
- Рассмотреть возможность использования более мелкозернистой синхронизации
- Увеличить размер тестового изображения для демонстрации ускорения
- Оптимизировать этап объединения диагональных пар