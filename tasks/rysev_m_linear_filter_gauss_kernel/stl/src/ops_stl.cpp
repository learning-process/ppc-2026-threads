...
RysevMGaussFilterSTL::RysevMGaussFilterSTL(const InType &in) {
  SetTypeOfTask(GetStaticTypeOfTask());
  GetInput() = in;
  GetOutput() = 0;
}

bool RysevMGaussFilterSTL::ValidationImpl() {
  return GetInput() >= 0 && GetOutput() == 0;
}

bool RysevMGaussFilterSTL::PreProcessingImpl() {
  if (GetInput() == 0) {
    std::string abs_path =
...
void RysevMGaussFilterSTL::ApplyKernelToChannel(int channel, int rows, int cols) {
  ...
  if (rows > 0) {
    num_threads = std::min(num_threads, static_cast<unsigned int>(rows));
  }
  ...
  for (unsigned int thread_idx = 0; thread_idx < num_threads; ++thread_idx) {
    int end_row = start_row + rows_per_thread + (std::cmp_less(static_cast<int>(thread_idx), remainder) ? 1 : 0);
...
bool RysevMGaussFilterSTL::RunImpl() {
  int rows = height_;
  int cols = width_;
  int ch = channels_;
...
bool RysevMGaussFilterSTL::PostProcessingImpl() {
  int64_t total = 0;
