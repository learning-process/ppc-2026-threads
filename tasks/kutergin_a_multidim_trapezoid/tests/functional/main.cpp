...
class KuterginATrapezoidFuncTest
    : public ppc::util::BaseRunFuncTests<InType, OutType, TestType> {
 public:
  using ParamType =
      typename ppc::util::BaseRunFuncTests<InType, OutType,
                                           TestType>::ParamType;
  ...
  const auto& test_data =
      std::get<static_cast<std::size_t>(
          ppc::util::GTestParamIndex::kTestParams)>(full_param);
 ...
 protected:
  ...
  InType GetTestInputData() final { return in_data_; }

  bool CheckTestOutputData(OutType& output) final {
    constexpr double tolerance = 1e-3;
    return std::abs(output - ref_value_) <= tolerance;
  }
...
const std::array<TestType, 12> kFunctionalTests = {{
    ...
    {"Const_2D_rect",
     {f_const, {{0.0, 2.0}, {1.0, 3.0}}, 100},
     Volume({{0.0, 2.0}, {1.0, 3.0}})},

    {"Linear_1D",
     {[](const std::vector<double>& x) { return x[0]; }, {{-1.0, 2.0}}, 200},
     IntLinear(-1.0, 2.0)},

    {"Linear_2D_sum",
     {f_sum2, {{0.0, 1.0}, {0.0, 1.0}}, 100},
     2.0 * IntLinear(0.0, 1.0) * 1.0},

    {"Linear_3D_sum",
     {f_sum3, {{0.0, 1.0}, {0.0, 1.0}, {0.0, 1.0}}, 60},
     3.0 * IntLinear(0.0, 1.0)},

    {"Square_1D",
     {f_square, {{0.0, 2.0}}, 200},
     IntSquare(0.0, 2.0)},
    ...
    {"Product_2D",
     {f_mul2, {{0.0, 2.0}, {0.0, 3.0}}, 150},
     IntLinear(0.0, 2.0) * IntLinear(0.0, 3.0)},

    {"Sin_0_pi",
     {f_sin, {{0.0, std::numbers::pi}}, 250},
     IntSin(0.0, std::numbers::pi)},
    ...
    {"SinSin_2D",
     {[](const std::vector<double>& x) {
      ...
      {{0.0, std::numbers::pi}, {0.0, std::numbers::pi}},
      180},
...
const auto kTaskPack =
    std::tuple_cat(ppc::util::AddFuncTask<KuterginAMultidimTrapezoidSEQ, InType>(
        kFunctionalTests, PPC_SETTINGS_kutergin_a_multidim_trapezoid));

const auto kValues = ppc::util::ExpandToValues(kTaskPack);

INSTANTIATE_TEST_SUITE_P(KuterginATrapezoidSuite,
                         KuterginATrapezoidFuncTest,
                         kValues,
