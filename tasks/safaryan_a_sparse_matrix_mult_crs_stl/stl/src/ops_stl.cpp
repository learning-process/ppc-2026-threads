...
SafaryanATaskSTL::SafaryanATaskSTL(const InType &in) {
  SetTypeOfTask(GetStaticTypeOfTask());
  GetInput() = in;
  GetOutput() = SparseMatrixCCS();
}

bool SafaryanATaskSTL::IsMatrixValid(const SparseMatrixCCS &matrix) {
...
bool SafaryanATaskSTL::ValidationImpl() {
...
bool SafaryanATaskSTL::PreProcessingImpl() {
  const auto &[a, b] = GetInput();
...
SparseMatrixCCS SafaryanATaskSTL::MultiplyMatrices(const SparseMatrixCCS &a, const SparseMatrixCCS &b) {
  ...
  for (int chunk_id = 0; chunk_id < chunks_count; ++chunk_id) {
    threads.emplace_back(
        [&, chunk_id]() { parts[chunk_id] = BuildColumnRange(a, b, begin_cols[chunk_id], begin_cols[chunk_id + 1]); });
  }
...
bool SafaryanATaskSTL::PostProcessingImpl() {
  return true;
}
