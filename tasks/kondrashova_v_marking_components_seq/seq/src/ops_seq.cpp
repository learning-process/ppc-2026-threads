...
bool KondrashovaVTaskSEQ::ValidationImpl() { return true; }
...
void BfsComponent(int start_i, int start_j, int width, int height, int label,
  ...
  labels_1d[static_cast<size_t>(start_i) * static_cast<size_t>(width) +
            static_cast<size_t>(start_j)] = label;
  ...
  while (!q.empty()) {
    ...
    for (int ki = 0; ki < 4; ++ki) {
      ...
      auto nidx = static_cast<size_t>(nx) * static_cast<size_t>(width) +
