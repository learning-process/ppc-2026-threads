...
#pragma omp parallel for schedule(static) default(none) shared(nthreads, chunk, n) num_threads(nthreads)
  ...
  for (int thread_idx = 0; thread_idx < nthreads; ++thread_idx) {
    ...
    int hi = std::min(lo + chunk, n);
    if (lo < n) {
      blocks[thread_idx].assign(GetOutput().begin() + lo, GetOutput().begin() + hi);
    }
  }
