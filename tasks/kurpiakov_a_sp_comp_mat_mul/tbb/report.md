...
Диапазон строк $[0, rows)$ делится на поддиапазоны `tbb::blocked_range<int>`. Каждый поддиапазон обрабатывается независимо, поэтому гонки отсутствуют.
...
**Контроль конкуренции:** `global_control` не используется; число worker‑потоков определяется runtime (без явного ограничения в коде).
