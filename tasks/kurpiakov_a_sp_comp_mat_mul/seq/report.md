...
## 1. Контекст
SEQ‑версия задает baseline корректности и времени для всех параллельных реализаций. Реализация полностью последовательная, без параллельных участков.
...
2. Для каждого ненулевого элемента $A(i, k)$ обходится строка $k$ матрицы $B$; сумма по колонкам накапливается в `row_acc`.  
